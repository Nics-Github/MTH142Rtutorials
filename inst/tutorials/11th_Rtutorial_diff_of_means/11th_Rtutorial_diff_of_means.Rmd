---
title: "Tutorial 11: Difference of Means t.test()"
author: "Nicholas Schwab"
output: learnr::tutorial
runtime: shiny_prerendered
description: "This tutorial walks through a difference of means t.test()"
---

```{r setup, include=FALSE}
library(learnr)
library(readxl)
Problem_1_data <- read_excel("./Problem-1-data.xlsx")
library(mosaic)
library(openintro)
...
[Here is the video](https://youtu.be/PMdiTji8AIA) of me walking through this tutorial.

## Diff of means with two vectors containing data

1.  Import data from a homework problem. I've done this in the background for this tutorial, if you need a refresher see Rtutorial 4 or ask for one in the Rtutorial help forum. This problem comes from HW 7 part 4. I've named the Data `Problem_1_data`. Here's the text from that problem from the OpenIntro Book.
...
3.  Take a look at the data again. Notice that `Men` and `Women` are in the same column in the variable `division`. These are called the levels of `division`. We discussed levels in the 3rd_Rtutorial. We'll use them to run our test.

```{r nyc-t-test,exercise=TRUE}
t.test(time_hrs~division,data=nyc_marathon, mu=0,alternative="two.sided")
```

There's scientific notation again, turn it off with `options(scipen=10)` or recognize that number rounds to 0.
...
Here is the [outline](https://github.com/Nics-Github/MTH142Rtutorials/blob/master/inst/tutorials/11th_Rtutorial_diff_or_means/11th_Rtutorial_outline.txt). Copy and paste it from here and paste it into a new .rmd file.
